...
def get_api_answer(current_timestamp):
    ...
    try:
        ...
        raise exceptions.YandexError(
            f'Ошибка соединения с Яндекс эндпоинт ошибка ConnectionError, '
            f'параметры {params}')
    except Exception as error:
        raise exceptions.YandexError(
...
def check_tokens():
    """Проеверка доступности переменных окружения."""
...
def main():
    ...
    while True:
        try:
            ...
            error = yandex_error
            current_timestamp = write_log_err(error, last_error, bot)
            ...
            current_timestamp = write_log_err(error, last_error, False)
